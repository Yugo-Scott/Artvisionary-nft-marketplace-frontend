...
const Create = () => {
  ...
  const uploadToArweave = async (url) => {
    ...
    // nftData for backend and to MongoDB
    const nftData = {
      ...
      tokenURI: tokenURI,
      seller: userId,
      owner: "0xCD60F93430a45ce43F9F21c247ee906BB36bA1C0",
    };

    mintNFT(tokenURI.data.id);
    sendDataToMongoDB(nftData);
  };

  // define method to send nftData to backend and to MongoDB
  ...
  const sendDataToMongoDB = async () => {
    try {
      ...
      console.error("Error posting NFT data:", error.response.data.message);
    }
  };
